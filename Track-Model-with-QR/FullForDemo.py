...
QR_CAM_INDEX = 2            # QR scanning camera
TABLE_A_CAM_INDEX = 1       # snapshot camera for Table A
TABLE_B_CAM_INDEX = 3       # snapshot camera for Table B (unused for now)
...
TABLES = {
    "Table A": (258,177,335,304),
    #"Table B": (0, 0, 0, 0),
}

QR_UPDATE_ZONE = (355,139,393,222)
...
_psu_q: "SimpleQueue[_UpdateJob | None]" = SimpleQueue()

def _psu_worker():
    s = requests.Session()
    while True:
        job = _psu_q.get()
        if job is None:  # optional shutdown signal
            break
        try:
            s.post(SESSIONS_URL, json=job.payload, timeout=0.5)
        except Exception:
            pass

_psu_thread = threading.Thread(target=_psu_worker, name="SessionUpdateWorker", daemon=True)
_psu_thread.start()
...
def push_session_update(track_id: int, cart_items_list: list, status: str = "processing") -> None:
    """
    Build a payload for /sessions/update from a list of InvoiceItem objects.
    - Aggregates duplicate product names by summing quantities.
    - Validates names/quantities.
    - Sends customer_id (snake_case) to match the API schema.
    - Omits 'cart' entirely if there are no valid items.
    """
    # Aggregate quantities by product name (InvoiceItem has .name and .quantity)
    cart_acc: dict[str, int] = defaultdict(int)
    for it in (cart_items_list or []):
        # Be defensive: tolerate plain dicts or objects with attributes
    ...
    # Only include 'cart' if we have at least one valid item
    if cart_acc:
        payload["cart"] = dict(cart_acc)

    # Attach identity using the correct snake_case key the API expects
    user_id = _get_user_id_for_track(track_id)  # your existing helper
    if user_id:
        payload["customer_id"] = user_id


    _psu_q.put(_UpdateJob(payload=payload))
...
def capture_snapshot(table_name: str) -> list[str]:
    """
    Capture a frame from the table camera, run object detection,
    and return a list of class names (duplicates allowed).
    """
...
def compute_missing(baseline: list[str], current: list[str]) -> list[str]:
    """
    Compare baseline and current lists and return items missing
    from the current list (duplicates preserved).
    """
...
def on_zone_exit(track_id: int, zone: str):
    ...
    if missing:
        ...
        for item_name in missing:
            try:
                # assumes queue_invoice_item(track_id, name) increments quantity internally
                queue_invoice_item(track_id, item_name)
            except Exception as e:
                print(f"[cart] Failed to queue missing item {item_name} for track {track_id}: {e}")
...
def queue_invoice_item(track_id: int, name: str):
    """
    Add one unit of `name` to the cart for this track.
    If the item exists, increment its quantity; otherwise add it with quantity=1.
    """
    try:
        # Ensure the cart exists for this track
        lst = cart_items.setdefault(track_id, [])

        # Look for an existing item with the same name
        for it in lst:
            if getattr(it, "name", None) == name:
                # Increment quantity in-place
                current_qty = int(getattr(it, "quantity", 0))
                it.quantity = current_qty + 1
                print(f"[cart] track {track_id}: +1 x {name} (qty now {it.quantity})")
                break
        else:
            # Not found: add a new item with quantity = 1
            new_item = InvoiceItem(name=name, quantity=1)
            lst.append(new_item)
            print(f"[cart] track {track_id}: +1 x {name} (added new)")
    except Exception as e:
...
def _flush_invoice_for_track(track_id: int):
    ...
    print(f"[leave] track {user_id} left the frame; attempting to flush invoice.")
...
def main():
    ...
    # prepare table cameras
    global cap_table_a, cap_table_b
    cap_table_a = cv2.VideoCapture(TABLE_A_CAM_INDEX, cv2.CAP_DSHOW)
    table_a_win = "Table A Cam"
    cv2.namedWindow(table_a_win)
    # table_b_win = "Table B Cam"  # if you later enable B
    if not cap_table_a.isOpened():
        raise RuntimeError("Could not open Table A camera (index 2).")
    ...
    for result in results_stream:
            ...
            for text, pts in zip(decoded_info, pts_list):
                ...
                if text:
                    ...
                    if  selected_track_id[0] is not None:
                        ...
                        try:
                            push_session_update(track_id, [], status="processing")
                        except Exception:
                            pass
        ...
        if ok_a:
            ...
            cv2.putText(placeholder, "Table A cam read failed", (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.imshow(table_a_win, placeholder)
